...
targetY = 4
p = 0.01
confidence_discretization=10
p_directions = {"west": round(0.1/3,2), "east": round(0.2/3,2), "south": round(0.3/3,2), "north": round(0.4/3,2)}
directions = ["west", "east", "south", "north"]
directions_effects = [
    "(xhatno'=max(xhatno-1, 0))",
    "(xhatno'=min(xhatno+1, N))",
    "(yhatno'=max(yhatno-1, 0))",
    "(yhatno'=min(yhatno+1, N))",
]
obstacles = []
...
def preambel():
    with open(prism_file, "a") as f:
        ...
        f.write(f"const int RES = {confidence_discretization-1};\n")
        for dir in directions:
            f.write(f"const double p_{dir} = {p_directions[dir]};\n")
...
def robot():
    with open(prism_file, "a") as f:
        ...
        f.write(
            "  [east] (move_ready=1) -> \n"
            f"    (1-3*p_east): (x'=min(x+1, N)) & (move_ready'=0) + \n"
            f"    p_east: (y'=min(y+1, N)) & (move_ready'=0) + \n"
            f"    p_east: (y'=max(y-1, 0)) & (move_ready'=0) + \n"
            f"    p_east: (x'=max(x-1, 0)) & (move_ready'=0); \n"
        )

        f.write(
            "  [west] (move_ready=1) -> \n"
            "    p_west: (x'=min(x+1, N)) & (move_ready'=0) + \n"
            "    p_west: (y'=min(y+1, N)) & (move_ready'=0) + \n"
            "    p_west: (y'=max(y-1, 0)) & (move_ready'=0) + \n"
            "    (1-3*p_west): (x'=max(x-1, 0)) & (move_ready'=0); \n"
        )

        f.write(
            "  [north] (move_ready=1) -> \n"
            "    p_north: (x'=min(x+1, N)) & (move_ready'=0) + \n"
            "    (1-3*p_north): (y'=min(y+1, N)) & (move_ready'=0) + \n"
            "    p_north: (y'=max(y-1, 0)) & (move_ready'=0) + \n"
            "    p_north: (x'=max(x-1, 0)) & (move_ready'=0); \n"
        )

        f.write(
            "  [south] (move_ready=1) -> \n"
            "    p_south: (x'=min(x+1, N)) & (move_ready'=0) + \n"
            "    p_south: (y'=min(y+1, N)) & (move_ready'=0) + \n"
            "    (1-3*p_south): (y'=max(y-1, 0)) & (move_ready'=0) + \n"
            "    p_south: (x'=max(x-1, 0)) & (move_ready'=0); \n"
        )
        f.write("\n")
        f.write(
        ...
        for x in range(mapSize):
            for y in range(mapSize):
                ...
                if direction < 4:
                    ...
                    f.write("(xhatno={0}) & (yhatno={1}) -> true;\n".format(str(x), str(y)))
        f.write("endmodule\n\n")
...
def knowledge(uncertainty_aware=False):
    with open(prism_file, "a") as f:
        ...
        f.write("  xhatno : [0..N] init xstart;\n")
        f.write("  yhatno : [0..N] init ystart;\n")
        if uncertainty_aware:
            f.write("  confidencehat : [0..RES] init RES;\n")
        ...
        if uncertainty_aware:
            for d, effect in zip(directions, directions_effects):
                f.write(f"  [{d}] ready=1 -> {effect} & (ready'=0) & (confidencehat'=min(RES, floor( (confidencehat*1.0/RES) * (1-3*p_{d}) * RES + 0.5 )));\n")
        else:
            for d, effect in zip(directions, directions_effects):
                f.write("  [{0}] ready=1 -> {1} & (ready'=0);\n".format(d, effect))

        f.write(
            "  [update] step>=c & ready=0 -> (xhatno'=x) & (yhatno'=y) & (step'=1) & (ready'=1);\n"
        )
